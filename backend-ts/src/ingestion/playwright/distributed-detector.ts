export function detectDistributed() {
  ...
  return {
    machine_id: env.MACHINE_ID || env.HOSTNAME || env.COMPUTERNAME || "local",
    ci_provider: ci,
    ci_run_id:
      env.GITHUB_RUN_ID ||
    ...
    ci_worker:
      ...
      null,
    shard_index: env.PLAYWRIGHT_SHARD_INDEX ? Number(env.PLAYWRIGHT_SHARD_INDEX) : 0,
    shard_total: env.PLAYWRIGHT_SHARD_TOTAL ? Number(env.PLAYWRIGHT_SHARD_TOTAL) : 1
  };
}
