...
export default function ReleaseStabilityMatrix({
  snapshot,
}: {
  snapshot: StabilitySnapshot;
}) {
  const [sortKey, setSortKey] = useState<SortKey>("requirementId");
  ...
  return (
    <div className="p-4 bg-white dark:bg-slate-900 shadow rounded space-y-3">
      <div className="flex items-center justify-between mb-1">
        <div className="text-lg font-semibold">Stability Matrix</div>
        <div className="text-xs text-slate-500">
          {requirements.length} requirements · {healingSignals.length} healing
          signals
        </div>
      </div>
      ...
      <div className="border border-slate-200 dark:border-slate-800 rounded overflow-hidden">
        <div className="grid grid-cols-12 gap-3 px-3 py-2 text-[11px] font-semibold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 sticky top-0 z-10">
          <button className="col-span-1 text-left"> </button>
        ...
        <div
          ref={parentRef}
          className="h-[360px] overflow-auto bg-white dark:bg-slate-900"
        >
          <div
            style={{
          ...
          >
            {rowVirtualizer.getVirtualItems().map((virtualRow) => {
              const requirement: RequirementStability =
                sorted[virtualRow.index];
              ...
              return (
                ...
                >
                  <MatrixRow
                    ...
                    healingSignals={healingSignals as HealingSignal[]}
